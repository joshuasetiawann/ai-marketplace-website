...
export default function Explore() {
  ...
  // Keep the query input synced if the URL changes (e.g. from navbar search).
  useEffect(() => {
  ...
  const results = useMemo(() => {
    ...
    switch (sort) {
      ...
      case 'newest':
        ...
        break
      default:
        list.sort((a, b) => (b.badge === 'trending' ? 1 : 0) - (a.badge === 'trending' ? 1 : 0) || b.reviews - a.reviews)
    }
    return list
  ...
  const FiltersPanel = (
    <div className="flex flex-col gap-7">
      <div className="flex items-center justify-between">
        <h3 className="font-display text-title-md text-on-surface">Filters</h3>
        {activeFilterCount > 0 && (
          <button onClick={clearAll} className="text-label-md font-label text-primary-container hover:text-primary transition-colors">
            Clear
          </button>
        )}
      </div>
          ...
          {CATEGORIES.map((c) => (
            <button
              ...
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-body-sm transition-colors ${
                category === c.id ? 'bg-primary-container/10 text-primary-container' : 'text-on-surface-variant hover:bg-white/5 hover:text-on-surface'
              }`}
            >
  ...
  return (
    <div className="max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-10">
      <div className="mb-8">
        <h1 className="font-display text-headline-md md:text-headline-lg text-on-surface mb-2">Explore Models</h1>
        <p className="text-body-md text-on-surface-variant">
          Discover premium AI models curated for high-performance applications.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="lg:hidden btn-soft px-4 py-3 relative"
          >
            <Icon name="tune" size={20} /> Filters
            {activeFilterCount > 0 && (
              <span className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-primary-container text-on-primary-container text-[11px] font-bold flex items-center justify-center">
                {activeFilterCount}
              </span>
            )}
          </button>
          <div className="relative">
            ...
            <select
              ...
              className="appearance-none bg-surface-container-lowest border border-white/10 rounded-full pl-10 pr-10 py-3 text-body-sm text-on-surface outline-none focus:border-primary-container/50 cursor-pointer h-full"
            >
              {SORTS.map((s) => (
      ...
      <div className="flex gap-8">
        ...
        <aside className="hidden lg:block w-64 shrink-0">
          <div className="sticky top-24 surface-card rounded-xl p-5">{FiltersPanel}</div>
        </aside>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-5">
            <p className="text-body-sm text-on-surface-variant">
              <span className="text-on-surface font-semibold">{results.length}</span> model{results.length !== 1 ? 's' : ''} found
            </p>
          </div>
          {results.length ? (
            ...
            <EmptyState
              ...
              action={
                <button onClick={clearAll} className="btn-primary px-6 py-3">
                  <Icon name="restart_alt" size={18} /> Reset filters
                </button>
              }
            />
          )}
        ...
        <div className="fixed inset-0 z-[65] lg:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-[85%] max-w-sm glass-nav border-r border-white/10 p-6 overflow-y-auto animate-slide-in-right">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-display text-title-md">Filters</h3>
              <button onClick={() => setFiltersOpen(false)} className="p-2 text-on-surface-variant"><Icon name="close" size={24} /></button>
            </div>
            {FiltersPanel}
            <button onClick={() => setFiltersOpen(false)} className="btn-primary w-full py-3 mt-8 sticky bottom-0">
...
function FilterGroup({ label, children }) {
  return (
    <div>
      <p className="font-label text-label-sm uppercase tracking-wider text-outline mb-3">{label}</p>
      {children}
    </div>
  )
}

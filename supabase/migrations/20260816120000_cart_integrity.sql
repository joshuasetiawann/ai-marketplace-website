-- ─────────────────────────────────────────────────────────────────────────────
-- Cart integrity (audit 2026-08-16)
--   • BUG-02: a real gateway emptied the cart BEFORE the buyer had paid
--   • BUG-04: cart quantity had no upper bound
-- ─────────────────────────────────────────────────────────────────────────────

-- ── 1. Quantity ceiling ──────────────────────────────────────────────────────
-- The only choke point every writer routes through. Server Actions clamp too
-- (lib/actions/commerce.ts) so the UI reports a cap instead of a DB error, but
-- the constraint is what makes the bound unbypassable: price × qty must stay
-- inside orders.total_usd's numeric(10,2).
update cart_items set qty = 99 where qty > 99;
alter table cart_items add constraint cart_items_qty_max check (qty <= 99);

-- ── 2. Don't empty the cart until the money actually lands ───────────────────
-- create_pending_order() deleted the cart, then handed the buyer off to the
-- provider's payment page. Cancelling, timing out or closing the tab therefore
-- cost the buyer their whole cart and left an orphan 'pending' order, with no
-- recovery path in the UI. The delete belongs in finalize_order(), which the
-- webhook calls once the payment is confirmed.
--
-- checkout() (the simulated provider) is untouched: it creates a PAID order and
-- clears the cart in the same transaction, which was always correct.
...
begin
  ...
  -- cart is deliberately left intact — see finalize_order()
  return v_order;
end;
$$;
...
-- Settle a pending order, book the seller split, and only now clear the lines
-- the buyer actually paid for. Still idempotent + race-safe: the conditional
-- UPDATE fires once, and everything after it is scoped to that single order.
-- The delete matches this order's items rather than truncating the cart, so
-- anything added while the payment was in flight survives.

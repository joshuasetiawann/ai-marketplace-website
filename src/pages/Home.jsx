...
export default function Home() {
  ...
  const [activeUse, setActiveUse] = useState('creative')
  ...
  return (
    <div className="overflow-hidden">
      {/* ───────────────── Hero ───────────────── */}
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop pt-16 md:pt-24 pb-16">
        ...
        <div className="flex flex-col items-center text-center gap-6 animate-slide-up">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
            ...
            <span className="font-label text-label-sm uppercase tracking-widest text-on-surface-variant text-[11px]">
              Next-Gen Intelligence Marketplace
            </span>
          </div>

          <h1 className="font-display text-[40px] leading-[1.05] sm:text-[56px] md:text-display-lg text-on-surface max-w-4xl">
            Find The <span className="text-gradient">AI</span> That Fits Your Life
          </h1>
          <p className="text-body-lg text-on-surface-variant max-w-2xl">
            The premium digital marketplace for curated, high-performance artificial intelligence models — tailored for
            business, creative, and everyday needs.
          </p>
          ...
          <form onSubmit={submit} className="w-full max-w-2xl mt-4 relative group">
            ...
            <input
              ...
              className="w-full bg-surface-container-lowest border border-white/10 rounded-full py-4 pl-14 pr-32 text-body-md text-on-surface placeholder:text-outline/70 outline-none focus:border-primary-container/50 transition-colors"
            />
          ...
          <div className="flex flex-wrap justify-center gap-2.5 mt-3">
            {USE_CASES.map((u) => (
              <Link
                key={u.id}
                to={`/explore?use=${u.id}`}
                className={`chip ${u.id === 'creative' ? 'chip-active' : ''}`}
              >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-px mt-16 rounded-xl overflow-hidden border border-white/[0.07] glass-panel">
          {STATS.map((s) => (
            <div key={s.label} className="px-6 py-6 text-center bg-surface/30">
      ...
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        <SectionHeading
          ...
          subtitle="The most powerful and popular intelligence right now."
          action={
            <Link to="/explore" className="btn-ghost px-5 py-2.5 self-start md:self-auto">
      ...
      {/* ───────────────── Categories ───────────────── */}
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        <SectionHeading eyebrow="Browse" title="Explore by Category" className="mb-10" />
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {CATEGORIES.filter((c) => c.id !== 'all').map((c) => (
            <Link
              key={c.id}
              to={`/explore?cat=${c.id}`}
              className="group surface-card rounded-xl p-5 flex flex-col items-center text-center gap-3 hover:border-primary-container/30 electric-glow-hover transition-all"
            >
              <span className="w-12 h-12 rounded-lg bg-primary-container/10 border border-primary-container/20 flex items-center justify-center text-primary-container group-hover:scale-110 transition-transform">
        ...
        <div className="glass-panel rounded-2xl overflow-hidden relative grid md:grid-cols-2 min-h-[420px]">
          <div className="p-8 md:p-14 flex flex-col justify-center">
            <div className="flex items-center gap-2 mb-5">
              <span className="font-label text-label-sm uppercase tracking-widest text-primary-container">Flagship Product</span>
              <span className="h-px w-12 bg-primary-container/30" />
            </div>
            <h2 className="font-display text-headline-md md:text-[44px] md:leading-[1.1] text-on-surface mb-4">
              Nexora <span className="text-primary-container font-light italic">Core Engine</span>
            </h2>
            <p className="text-body-lg text-on-surface-variant mb-8 max-w-md">
              Our foundational multimodal model. Seamlessly integrate text, image, and voice intelligence into your
              workflow with unprecedented latency.
            </p>
            <div className="flex flex-wrap gap-3">
              <Link to="/model/nexus-vision-pro" className="btn-primary px-6 py-3">
                Explore Engine <Icon name="arrow_forward" size={18} />
              </Link>
              <Link to="/pricing" className="btn-ghost px-6 py-3">
                View Pricing
              </Link>
            </div>
          </div>
            ...
            <div className="relative w-60 h-60">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                <Icon
                  ...
                  style={{ filter: 'drop-shadow(0 0 16px rgba(0,229,255,0.8))' }}
                />
              </div>
            </div>
          </div>
      ...
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        <SectionHeading
          eyebrow="Fresh"
          title="New Arrivals"
          subtitle="Recently published intelligence from top creators."
          action={
            <Link to="/explore" className="btn-ghost px-5 py-2.5 self-start md:self-auto">
      ...
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        <SectionHeading
          ...
          title="Top Creators"
          action={
            <Link to="/creators" className="btn-ghost px-5 py-2.5 self-start md:self-auto">
              All Creators <Icon name="arrow_forward" size={18} />
            </Link>
          }
          className="mb-10"
        />
        <div className="grid sm:grid-cols-3 gap-gutter">
          {Object.values(CREATORS).map((c) => (
            <Link
              key={c.id}
              to={`/creator/${c.id}`}
              className="glass-panel rounded-xl p-6 flex items-center gap-4 hover:border-primary-container/30 electric-glow-hover transition-all"
            >
              <span className="w-14 h-14 rounded-full overflow-hidden border border-white/10 shrink-0">
                <ModelArtwork seed={c.id} colors={c.art} className="w-full h-full" />
              </span>
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <p className="font-display text-body-lg font-semibold text-on-surface truncate">{c.name}</p>
                  {c.verified && <Icon name="verified" size={16} className="text-primary-container shrink-0" fill />}
                </div>
                <p className="text-body-sm text-on-surface-variant truncate">{c.title}</p>
                <p className="text-[12px] text-outline mt-1">{c.stats.followers} followers</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="relative max-w-container-max mx-auto px-margin-mobile md:px-margin-desktop py-12">
        <div className="relative rounded-2xl overflow-hidden border border-primary-container/20 bg-gradient-to-br from-surface-container to-surface-container-lowest p-10 md:p-16 text-center">
          ...
          <h2 className="font-display text-headline-md md:text-headline-lg text-on-surface mb-4 max-w-2xl mx-auto">
            Build, publish & earn with your own AI models
          </h2>
          <p className="text-body-lg text-on-surface-variant max-w-xl mx-auto mb-8">
            Join thousands of creators monetizing their intelligence on the most premium marketplace in the industry.
          </p>
          <div className="flex flex-wrap justify-center gap-3">
            <Link to="/upload" className="btn-primary px-7 py-3.5">
              Start Selling <Icon name="rocket_launch" size={18} />
            </Link>
            <Link to="/about" className="btn-ghost px-7 py-3.5">
              Learn More
            </Link>
          </div>
        </div>
